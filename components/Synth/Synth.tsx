...
const Synth: React.FC = () => {
  ...
  useEffect(() => {
    document.addEventListener("DOMContentLoaded", loadSynth);
    document.addEventListener("beforeunload", unloadSynth);

    return () => {
      document.removeEventListener("DOMContentLoaded", loadSynth);
      document.removeEventListener("beforeunload", unloadSynth);
    };
  }, []);
